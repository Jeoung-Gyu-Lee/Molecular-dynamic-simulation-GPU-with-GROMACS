...
# mamba create -n amber -c conda-forge python=3.11 ambertools hdf5 openbabel acpype gmx_MMPBSA -y
...
# printf "1\n13\n" | gmx mindist -s md.tpr -f md_fit.xtc -n index.ndx -od mindist.xvg -d 0.35
...
# Commented out IPython magic to ensure Python compatibility.
# #@title **MM/PBSA Binding Free Energy Analysis**
# # Verify trajectory frame count
# %%bash
# source /usr/local/gromacs/bin/GMXRC
# echo "===== Trajectory info ====="
# gmx check -f md_fit.xtc 2>&1 | grep -E "Step|Last frame|Coords"
# 
# echo ""
# echo "===== Index groups ====="
# echo "q" | gmx make_ndx -f md.tpr -n index.ndx -o index_check.ndx 2>&1 | grep -E "^\s*[0-9]+\s"
...
# source /usr/local/gromacs/bin/GMXRC
# cat > mmpbsa.in << 'EOF'
# Input file for MM/PBSA with Interaction Entropy
# &general
# sys_name="Protein_Ligand_Complex",
# startframe=8001, endframe=10000, interval=10, # check the convergence region of each ligand, sampling interval: 10 ps = 0.01 ns
# verbose=2, keep_files=0,
# forcefields="oldff/leaprc.ff14SB, leaprc.gaff2",
# PBRadii=3,
# temperature=310.15,
...
# /
# &pb
# istrng=0.150, fillratio=4.0, radiopt=0, inp=1,
# /
# EOF
# 
# cat mmpbsa.in

# Patch for 'np.float' on the'numpy1.24+'
!grep -rn "np\.float\b\|np\.int\b\|np\.bool\b" /usr/local/envs/amber/lib/python3.11/site-packages/GMXMMPBSA/
!sed -i 's/np\.float\b/float/g' \
    /usr/local/envs/amber/lib/python3.11/site-packages/GMXMMPBSA/amber_outputs.py \
    /usr/local/envs/amber/lib/python3.11/site-packages/GMXMMPBSA/calculation.py

# Commented out IPython magic to ensure Python compatibility.
# %%bash
# source /usr/local/gromacs/bin/GMXRC
# export PATH="/usr/local/envs/amber/bin:$PATH"
# export LD_LIBRARY_PATH="/usr/local/envs/amber/lib:$LD_LIBRARY_PATH"
# 
# WORK=/content/mmpbsa_work
# mkdir -p $WORK && cd $WORK
# 
# # Copy the required files
# SRC=/content/drive/MyDrive/Epicatechingallate
# cp $SRC/md.tpr $SRC/md_fit.xtc $SRC/index.ndx $SRC/topol.top \
#    $SRC/mmpbsa.in $SRC/UNL_bcc_gaff2.mol2 $SRC/UNL_GMX_nonum.itp \
#    $SRC/*.itp $WORK/ 2>/dev/null
# 
# NP=$(( $(nproc) < 4 ? $(nproc) : 4 ))
# rm -f _GMXMMPBSA_* reference.frc *.mdout *.mdin
# 
# # Run MM-PBSA (ignore the non-zero exit caused by missing PyQt5), 1 = protein, 13 = UNL (ligand)
# mpirun -np $NP gmx_MMPBSA -O \
#     -i mmpbsa.in \
#     -cs md.tpr \
#     -ci index.ndx \
#     -cg 1 13 \
#     -ct md_fit.xtc \
#     -cp topol.top \
#     -lm UNL_bcc_gaff2.mol2 \
#     -o FINAL_RESULTS_MMPBSA.dat \
#     -eo FINAL_RESULTS_MMPBSA.csv 2>&1 | tee mmpbsa_run.log

# Commented out IPython magic to ensure Python compatibility.
# %%bash
# SRC=/content/drive/MyDrive/Epicatechingallate
# DEST=$SRC/mmpbsa_results_$(date +%Y%m%d_%H%M%S)
# mkdir -p $DEST
# 
# cp -v /content/mmpbsa_work/FINAL_RESULTS_MMPBSA.dat  $DEST/
# cp -v /content/mmpbsa_work/FINAL_RESULTS_MMPBSA.csv  $DEST/
# cp -v /content/mmpbsa_work/RESULTS_gmx_MMPBSA.h5     $DEST/
# cp -v /content/mmpbsa_work/gmx_MMPBSA.log            $DEST/
# cp -v /content/mmpbsa_work/mmpbsa_run.log            $DEST/
# cp -v /content/mmpbsa_work/mmpbsa.in                 $DEST/
# 
# echo "= Done ="
# ls -la $DEST/